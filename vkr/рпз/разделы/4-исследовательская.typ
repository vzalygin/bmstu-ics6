...
- #определение([.NET CIL], [.NET CIL -- .NET Common Intermediate Language, промежуточный язык (байт-код) платформы .NET, в который транслируются программы и который исполняется средой выполнения (CLR) или преобразуется в машинный код]) и байт-код Java, которые также являются языками под стековые виртуальные машины (#определение([.NET CLR], [.NET CLR -- .NET Common Language Runtime, виртуальная машина платформы .NET, исполняющая программы на языке .NET CIL]) и JVM соответственно).
...
Байт-код WebAssembly (или, сокращённо, Wasm) спроектирован как низкоуровневая переносимая платформа с околонативной производительностью вычислений~@wasm-syntax. Поддерживаются два формата описания программ Wasm: бинарное представление, понимаемое виртуальной машиной Wasm, и текстовое представление в синтаксисе S-выражений, удобное для чтения человеком. В зависимости от цели одну и ту же программу можно представить в обоих вариантах.
