...
В 2006 году Кристофер Диггинс опубликовал несколько технических отчётов~@cat-a@cat-b, в рамках которых описывает алгоритм статической типизации для стековых языков, в качестве примера используя свой экспериментальный язык Cat. Cat наследует идею Joy: любая программа рассматривается как преобразование вида "stack -> stack", а основная операция~-- композиция таких преобразований, реализуемая простой конкатенацией лексем в исходном тексте программы. В плане синтаксиса Cat аналогичен предыдущим и использует обратную польскую запись~@cat-b. Грамматика показана на листинге~@ebnf-cat в виде #определение([РБНФ], [РБНФ -- расширенная форма Бекуса-Наура, нотация задания формальных грамматик, расширяющая БНФ средствами записи повторений, альтернатив, опциональных элементов и группирования]).
...
В развитии стековых языков прослеживается тенденция: изначально они работают на низких уровнях абстракции благодаря простоте #определение([рантайма], [Рантайм -- (англ. runtime, время исполнения) окружение программы, в котором она исполняется]) (по сравнению с нестековыми языками), высокой переносимости и оптимизациям, доступным при меньшей стоимости за счёт структуры программ и модели исполнения (например, нитевого кода Forth). Со временем стековые языки обзаводятся формальной базой, которая позволяет точно описывать работу программ и проводить развитый статический анализ и трансформации кода, нацеленные на уменьшение объёма потребляемой памяти и уменьшение скорости работы.
...
#let cl(content) = c[#align(left)[#content]]
