...
В 2006 году Кристофер Диггинс опубликовал несколько технических отчётов~@cat-a@cat-b, в рамках которых описывает алгоритм статической типизации для стековых языков, в качестве примера используя свой экспериментальный язык Cat. Cat наследует идею Joy: любая программа рассматривается как преобразование вида "stack -> stack", а основная операция~-- композиция таких преобразований, реализуемая простой конкатенацией лексем в исходном тексте программы. В плане синтаксиса Cat аналогичен предыдущим и использует обратную польскую запись~@cat-b. Грамматика показана на листинге~@ebnf-cat.
