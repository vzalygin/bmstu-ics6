...
- #определение([.NET CIL], [_.NET CIL_ -- .NET Common Intermediate Language, промежуточный язык (байткод) платформы .NET, в который транслируются программы и который исполняется средой выполнения (CLR) или преобразуется в машинный код]) и байткод Java, которые также являются языками под стековые виртуальные машины (#определение([.NET CLR], [_.NET CLR_ -- .NET Common Language Runtime, виртуальная машина платформы .NET, исполняющая программы на языке .NET CIL]) и JVM соответственно).
...
Forth~-- первый известный стековый конкатенативный язык, созданный Чарльзом Муром в начале 1970-х годов как расширяемый язык и интерактивная среда: ключевые слова можно добавлять прямо во время работы системы и таким образом формировать собственное подмножество языка под конкретную предметную область~@forth-page. Синтаксис основан на #определение([обратной польской], [_Обратная польская запись_ -- форма записи выражений, в которой операторы следуют за операндами (постфиксная нотация)]), или постфиксной, нотации: программа~-- это последовательность слов, разделённых пробелами. #определение([Интерпретатор], [_Интерпретатор_ -- программа, которая выполняет исходный код, покомандно анализируя и исполняя команды "на лету"]) идёт слева направо по программе и ищет #определение([лексемы], [_Лексема_ -- минимальная значимая единица исходного текста программы, выделяемая на этапе лексического анализа (например, идентификатор, ключевое слово, литерал, оператор, разделитель)]) в словаре доступных слов, после чего либо выполняет их, либо трактует как число и кладёт на стек. Язык и среда исполнения едины: система работает в режимах интерпретации и компиляции, переключаясь между ними. Для добавления нового слова или отложенных вычислений используется режим компиляции, для исполнения~-- режим интерпретации. Стандартной библиотеки в привычном смысле нет~-- функциональность наращивается подключаемыми наборами слов (word sets), зависящими от конкретной реализации. При этом ANS Forth (стандарт языка) задаёт базовый набор слов "core word set" со стековыми операциями, арифметикой, сравнениями, управлением потоком и примитивами памяти~@forth-standard. Система типов у Forth "бестиповая": данные представлены машинными словами фиксированной разрядности, а корректность трактовки содержимого стека контролирует программист, что повышает гибкость и предсказуемость времени выполнения за счёт единой семантики для любых данных, но усложняет статический анализ и проверку ошибок. Для Forth предложена оптимизация занимаемого программой пространства техникой "#определение([#h(-0.3em) нитевого кода], [_Нитевой код_ -- способ представления программы в виде последовательности ссылок (адресов) на подпрограммы или примитивы исполнения, используемый для ускорения интерпретации и уменьшения объёма кода в стековых системах])": программа по сути является последовательностью безусловных переходов, каждый из которых ведёт к инструкциям, выполняющим ту или иную команду. Работа с памятью осуществляется через явные операции чтения и записи по адресу, что даёт низкоуровневое управление. Область применения Forth~-- прошивки, встроенные и ресурсно-ограниченные системы, системы реального времени и космическая техника.
...
В 2006 году Кристофер Диггинс опубликовал несколько технических отчётов~@cat-a@cat-b, в рамках которых описывает алгоритм статической типизации для стековых языков, в качестве примера используя свой экспериментальный язык Cat. Cat наследует идею Joy: любая программа рассматривается как преобразование вида "stack -> stack", а основная операция~-- композиция таких преобразований, реализуемая простой конкатенацией лексем в исходном тексте программы. В плане синтаксиса Cat аналогичен предыдущим и использует обратную польскую запись~@cat-b. Грамматика показана на листинге~@ebnf-cat в виде #определение([РБНФ], [_РБНФ_ -- расширенная форма Бекуса-Наура, нотация задания формальных грамматик, расширяющая БНФ средствами записи повторений, альтернатив, опциональных элементов и группирования]).
...
В грамматике не показаны все возможности языка Cat. Полная версия языка также предполагает для определяемых функций возможность явного указания типов и встраиваемых комментариев-документации. Согласно требованиям технического задания в процессе разработки эти возможности опущены и компилятор поддерживает подмножество языка без использования этих конструкций.

Статическая типизация в контексте стековых языков подразумевает, что в момент компиляции возможно построить конфигурацию стека (его размер и "форму"~-- типы значений, которые на нём лежат) для произвольного момента времени исполнения. Таким образом, вывод типов позволяет проводить статический анализ и трансформации. Подробности статического вывода типов описываются в подразделе~@заголовок-вывод-типов. В языке присутствует несколько встроенных команд, их описание приведено в таблице~@команды-cat. Помимо них в язык встроены арифметические и логические команды, а также команды сравнения чисел. Язык использует три типа данных~-- числа, булевы значения и цитаты.
...
Ещё одним ответвлением от Joy стал язык Factor, впервые опубликованный в 2003 году Славой Пестовым~@factor-wiki. Преемственность Factor прослеживается по нескольким деталям: синтаксис на основе обратной польской записи, аналогичная идея использования и способ создания новых комбинаторов. Ключевая особенность~-- это первый прикладной высокоуровневый стековый язык общего назначения. Его можно так назвать благодаря богатой стандартной библиотеке (которая включает в себя, например, веб-фреймворк и парсер XML) и инструментарию разработки, собранному в одной IDE и включающему в себя #определение([компилятор], [_Компилятор_ -- программа, выполняющая преобразование исходного текста программы на языке программирования в эквивалентную форму представления (как правило, объектный код, машинный код или промежуточное представление), пригодную для последующего исполнения или дальнейшей трансляции]) с развитой системой анализов, интерактивный отладчик, браузер документации и инспектор объектов. Как и Joy, язык динамический, но  имеет отличие: при объявлении новой функции программист должен указать её тип, а компилятор, в свою очередь, должен проверить этот тип на соответствие содержимому функции. Внутренний механизм этой проверки описывается как абстрактная интерпретация программы: компилятор (а точнее, модуль "Stack-checker" внутри него) симулирует эффекты команды на абстрактном стеке, при встрече ветвлений анализирует обе ветви и унифицирует состояния, а несовместимость размера или формы стека превращает в ошибки времени компиляции~@factor-paper. На основе анализа типов компилятор проводит высокоуровневые и низкоуровневые оптимизации. Также дополнительно используются динамические проверки времени исполнения. За счет этих возможностей Factor можно назвать применимым для решения задач прикладного программирования.

Особое значение имеет спецификация WebAssembly~-- одноимённого стекового языка и одноимённой виртуальной машины, которые были разработаны и выпущены в 2017 году группой компаний W3C, Mozilla, Google, Microsoft, Apple и других. Wasm определяет низкоуровневый переносимый байткод, архитектуру и семантику набора команд для виртуальной стековой машины, запускаемой преимущественно (но не только) в веб-браузерах наравне с другими движками~@wasm-spec. В исходных целях проектирования Wasm зафиксированы требования к компактности двоичного представления, быстрой однопроходной валидации и компиляции, а также к "песочнице", пригодной для запуска недоверенного кода с производительностью низкоуровневого кода. Как стековый язык Wasm использует неявный стек данных: инструкции потребляют значения с вершины стека и помещают результаты обратно, при этом реализация не обязана хранить "настоящий" стек~-- спецификация описывает интерпретацию стека как набора анонимных регистров. Это первый из представленных в обзоре языков, который является целью компиляции, а не сам компилируется во что-либо. Поддержка компиляции в Wasm имеется для множества высокоуровневых языков, в первую очередь для полностью компилируемых языков C/C++/Rust. Для языка существует два представления: текстовое (формат "Wat" с синтаксисом на #определение([S-выражениях], [_S-выражение_ -- скобочный формат записи, в котором на первом месте внутри скобок стоит оператор, а затем перечисляются операнды])~@wasm-syntax), удобное для чтения человеком, и бинарное, понимаемое виртуальной машиной. Подробнее устройство байткода Wasm и модель его исполнения рассмотрены в подразделе~@заголовок-wasm.
...
Результаты сравнения синтаксиса языков представлены в таблице~@syntax-compare.
...
))[ Сравнение синтаксиса языков ] <syntax-compare>

В общем случае в рассмотренных языках прослеживается следующая закономерность. Для языков, которые используются непосредственно программистами для написания программ вручную, применяется характерный конкатенативный синтаксис, что соответствует изначально заложенным в языки этого множества концепциям минималистичности и простоты. Вероятно, некоторую роль в сохранении такой формы играет фактор историчности: исторически первый стековый язык (Forth) обладал конкатенативным синтаксисом.
...
В развитии стековых языков прослеживается тенденция: изначально они работают на низких уровнях абстракции благодаря простоте #определение([рантайма], [_Рантайм_ -- (англ. runtime, время исполнения) окружение программы, в котором она исполняется]) (по сравнению с нестековыми языками), высокой переносимости и оптимизациям, доступным при меньшей стоимости за счёт структуры программ и модели исполнения (например, нитевого кода Forth). Со временем стековые языки обзаводятся формальной базой, которая позволяет точно описывать работу программ и проводить развитый статический анализ и трансформации кода, нацеленные на уменьшение объёма потребляемой памяти и уменьшение скорости работы.
...
Байткод WebAssembly (или, сокращённо, Wasm) спроектирован как низкоуровневая переносимая платформа с околонативной производительностью вычислений~@wasm-syntax. Поддерживаются два формата описания программ Wasm: бинарное представление, понимаемое виртуальной машиной Wasm, и текстовое представление в синтаксисе S-выражений, удобное для чтения человеком. В зависимости от цели одну и ту же программу можно представить в обоих вариантах.

Единицей трансляции Wasm является модуль~-- описанный спецификацией Wasm файл, содержащий коды программ, наборы метаданных, информацию об импортах и экспортах и другие составляющие~@wasm-spec. Модуль является самостоятельной единицей, готовой к запуску на виртуальной машине. Для представления чисел используется формат little-endian (младшие разряды чисел записываются по младшим адресам). При кодировании векторов (последовательностей значений) в начале блока данных записываются 4 байта, задающие длину вектора, а затем кодируемые данные. Строки кодируются как байтовые векторы UTF-8.
...
- линейная память~-- непрерывные последовательности байтов;
...
#таблица(table(
  ...
  table.header(c[ № ], c[ Название ], c[ Описание ]),
  c[0], c[Custom section], cl[Игнорируется виртуальной машиной, хранит произвольные данные. Может использоваться для различных целей, в том числе для отладки.],
  c[1], c[Type section], cl[Объявления доступных для использования типов функций (типы параметров и типы результатов). Остальные секции ссылаются на данную при необходимости указать тип.],
  c[2], c[Import section], cl[Объявления сущностей, которые импортируются в модуль для работы. Определения импортируемых сущностей предоставляют модули, которые их экспортируют.],
  c[3], c[Function section], cl[Объявления типов для определенных в модуле функций, имеет параллельную с секцией Code нумерацию.],
  c[4], c[Table section], cl[Объявления таблиц ссылок, имеет параллельную с секцией Element нумерацию.],
  c[5], c[Memory section], cl[Объявления линейной памяти.],
  c[6], c[Global section], cl[Объявления глобальных переменных.],
  c[7], c[Export section], cl[Объявления публичных сущностей, которые могут быть импортированы другими модулями.],
  c[8], c[Start section], cl[Объявление индекса функции, которую необходимо вызвать при инстанцировании модуля.],
  c[9], c[Element section], cl[Объявления элементных сегментов~-- заполнения таблиц.],
  c[10], c[Code section], cl[Объявления тел функций.],
  c[11], c[Data section], cl[Объявление сегментов данных.],
  c[12], c[Data count section], cl[Опциональное указание числа сегментов данных (Data section), которое ускоряет валидацию модуля.],
  c[13], c[Tag section], cl[Объявления тегов для реализации механизма исключений.], 
))[ Описание секций модуля ] <wasm-секции>

Вызов функций возможен либо напрямую через указание индекса нужной функции, либо косвенно через таблицу ссылок. Для операций над данными используется стек данных. Чтобы выполнить некую операцию, необходимо положить на стек требуемые данные; операция берёт операнды со стека и кладёт на стек результат выполнения. Функции действуют аналогично. Согласно описанию в подразделе~@заголовок-вывод-типов, тип функции описывает преобразование верхушки стека, которое осуществляет функция.
...
Полученные сведения следует использовать при разработке компонента кодогенерации для указанного языка. Компиляция программ в Wasm перенимает преимущества у виртуальной машины Wasm в виде универсальности, высокой скорости работы, широкого круга поддерживаемых устройств -- все это делает Wasm удобной целью компиляции для универсальных программ, коими являются программы на языке Cat.
