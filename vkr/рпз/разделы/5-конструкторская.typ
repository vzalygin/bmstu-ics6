...
Для выполнения третьей задачи компилятора, оптимизации, было создано немало методов и техник. Оптимизация основывается на переписывании программы таким образом, чтобы наблюдаемые извне эффекты сохранились в неизменном виде, а сама программа стала более оптимальной. На текущий момент в индустрии компиляторов популярностью пользуется форма SSA представления (single static assignment, единственное статическое присвоение). SSA форма удобна для проведения большого количества оптимизаций, так как за счёт своих свойств заметно упрощает алгоритмы анализа и трансформации. Подробнее об SSA представлении рассказывается в разделе~@технология-ssa. Таким образом, ещё одно промежуточное представление~-- SSA форма ~-- закрывает последнюю задачу, стоящую перед компилятором.
/* TODO тут ссылку на Владимирова? */
