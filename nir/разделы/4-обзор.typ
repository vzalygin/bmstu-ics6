...
Одна из особенностей языка: почти все управляющие слова в Forth реализуются при помощи понятия слов времени компиляции -- это конструкции, которые во время компиляции разворачиваются в другие слова. Например, конструкция с ветвлением "... DUP 6 < IF DROP 5 ELSE 1 - THEN ..." разворачивается в последовательность слов "... DUP LIT 6 < ?BRANCH 5 DROP LIT 5 BRANCH 3 LIT 1 - ..." -- в результате конструкция условного перехода заменена на слова условного и безусловного переходов @forth-page.
...
Среди стековых языков Factor занимает нишу языков общего назначения @factor-paper. Первая версия языка была выпущена в 2003 году Славой Пестовым. Логотип языка показан на рисунке @factor-logo. Factor заимствует многие элементы из языка Joy:
...
Прикладным язык считается за счет большой стандартной библиотеки, которая позволяет решать широкий спектр прикладных задач. Благодаря статическому анализу (так называемый stack-checker), а также динамическим проверкам времени исполнения компилятор языка обеспечивает верификацию программ, а также высокоуровневые и низкоуровневые оптимизации, что также делает язык более применимым для большого количества продуктовых задач. Пример определения нового слова показан в листинге @factor-example.
