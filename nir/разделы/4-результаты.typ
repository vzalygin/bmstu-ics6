...
Таблица с сравнением языков по синтаксису представлена в таблице 1.
...
#таблица(table(
  ...
  block(breakable: false)[Повышенная человекочитаемость за счет иной грамматики записи],
))[ Сравнение синтаксиса языков ] <syntax-compare>

В общем случае в рассмотренных языках прослеживается следующая
закономерность. Для языков, которые используются непосредственно
программистами для написания программ вручную, применяется характерный
конкатенативный синтаксис, что соответствует изначально заложенным в
языки данного множества концепциям минималистичности и простоты.
Вероятно, некоторую роль в сохранении такой формы играет фактор историч-
ности: исторически первый стековый язык (Forth) обладал конкатенативным
синтаксисом.

В то же время синтаксис языков, которые являются целями трансляции,
представлен более широким разнообразием. В частности Wasm использует
S-выражения – тоже одна из достаточно распространенных форм синтаксиса
для функциональных языков. S-выражения также соответствуют концепции
простоты (РБНФ синтаксиса таких языков имеет относительно малый размер).

Таким образом, отличительная черта синтаксиса стековых языков
– минималистичный дизайн, который упрощает как механизмы разбора
программ за счет упрощенного парсера, так и работу программиста за
счет меньшего объема требуемых знаний о синтаксисе и высокой степени
читаемости (исходя из убеждения, что чем проще синтаксис, тем проще его
читать).

== Сравнение подходов к построению стандартной библиотеки

Таблица 2 описывает принципы подключения дополнительной
функциональности.
...
))[ Подходы к подключению дополнительной функциональности ] <stdlib-compare>

Здесь также можно выделить закономерность. Если язык предназначен
для некоего высокоуровневого взаимодействия, то он скорее имеет библиотеку,
пусть и не самую большую (Joy, Cat, Factor). Если же речь идет про язык
близкий к машине, то стандартной библиотеки у него нет, но есть иной способ
подключать дополнительную функциональность (Forth, Wasm).
...
#таблица(table(
  ...
  block(breakable: false)[Forth],
  block(breakable: false)[Безтиповой],
  block(breakable: false)[Проверки типовой согласованности не навязываются; корректность интерпретации содержимого стека обеспечивается соглашениями и выявляется на этапе исполнения.],

  block(breakable: false)[Joy],
  block(breakable: false)[Динамическая],
  block(breakable: false)[Динамическая проверка типов при выполнении],

  block(breakable: false)[Factor],
  block(breakable: false)[Динамическая],
  block(breakable: false)[Stack-checker как статический анализ: абстрактная интерпретация эффектов слов на абстрактном стеке и проверка согласованности ветвей; ошибки выявляются при компиляции.],

  block(breakable: false)[Cat],
  block(breakable: false)[Статическая (эффекты стека)],
  block(breakable: false)[Вывод типов и верификация формы стека, включая row-полиморфизм; гарантии согласованности ветвлений и инвариантов циклов.],

  block(breakable: false)[Wasm],
  block(breakable: false)[Статическая (нормативная)],
  block(breakable: false)[Однопроходная валидация модуля по типам и структурированному управлению; предотвращение underflow и несогласованности типов до исполнения.],
))[ Сравнение типизации ] <typing-analysis-compare>

Согласно таблице 3 со временем стековые языки программирования
обрастают возможностями динамического и статического анализа. Начав
от безтипового Forth, современные стековые языки предлагают мощный
инструментарий анализа и верификации программ. Вместе с уровнем анализа
растет также уровень безопасности программ на языках.
...
#таблица(table(
  ...
  text(size: 12pt)[Трансляция управляющих конструкций в примитивные переходы и последовательности слов, уменьшая накладные расходы управления.],
...
))[ Основные техники оптимизации в стековых языках ] <opt-table>

Оптимизации в стековых языках можно условно разделить на две
группы: преобразования представления и диспетчеризации команд (ускоряют
интерпретатор/виртуальную машину) и оптимизации, использующие резуль-
таты анализа семантики (стек-эффекты, типы, структура управления),
что позволяет убирать лишние проверки и преобразовывать программу
без нарушения корректности. Стековые языки изначально поддерживают
техники первой группы вследствие своей структуры (например, нитевой код
Forth). С развитием подходов к статическому анализу становятся доступны
оптимизации из второй группы – Cat, Wasm и другие более молодые
языки активно применяют оптимизации, основанные на эквивалентных
преобразованиях структуры программы, для увеличения скорости работы и
снижении размера веса программы.

Отдельный класс оптимизаций для стековых виртуальных машин –
stack caching: хранение части виртуального стека (обычно верхних элементов)
в регистрах реальной машины, чтобы сократить число загрузок/выгрузок и
обновлений указателя стека. В работах М. А. Эрлтля предложены и подробно
исследованы статический подход (компилятор отслеживает состояние кэша
как автомат) и динамический подход (несколько версий интерпретатора
для разных состояний кэша), а также показано, что stack caching хорошо
сочетается с динамическими суперинструкциями, которые уменьшают долю
времени на диспетчеризацию команд [13–15]. Эти техники применимы не
к одному конкретному языку, а к широкому классу реализаций стековых
интерпретаторов и виртуальных машин, в том числе к реализациям, близким
по модели исполнения к Forth, Factor и другим стековым системам.

== Сравнение языков по способу управления памятью

Участвующие в обзоре языки по-разному реализуют управление
памятью под динамические данные. Различия приведены в таблице @mem-compare. В целом
же можно отметить взаимосвязь: для низкоуровневых языков управление
памятью остается за пределами их ответственности (и ложится на плечи
программиста или реализуемого рантайма). Для высокоуровневых же типично
...
#таблица(table(
  ...
  [Cat],
  [Опора на целевую управляемую среду (.NET CIL).],
...
Области применения рассмотренных языков определяются тем, на какие
ограничения и требования изначально ориентирована их модель исполнения.
Forth применяется во встраиваемых и ресурсно-ограниченных системах и в
системах реального времени, где наиболее характерны малый размер среды,
предсказуемость поведения и возможность низкоуровневого управления
аппаратурой через минимальное ядро и расширяемый словарь слов. Joy имеет
преимущественно академическое и экспериментальное применение. Cat также
относится к исследовательским языкам и изучает возможности верификации
и оптимизации программ .NET CIL. Factor занимает нишу языка общего
назначения: для него наиболее характерны развитая стандартная библиотека,
инструменты разработки и оптимизирующая компиляция, при этом сохранён
конкатенативный стиль и используется статический контроль формы стека
(stack-checker) как средство повышения надёжности и производительности.
WebAssembly используется как переносимый низкоуровневый формат для
безопасного исполнения недоверенного кода, для него наиболее характерна
предсказуемая семантика и совместимость с различными исходными языками
и рантаймами.

В целом же в пути развития стековых языков видна явная тенденция:
изначально работающие низких уровнях абстракций благодаря простоте
рантайма (в сравнении с нестековыми языками), высокой переносимости
и оптимизациям, доступным за меньшую стоимость за счет структуры
программ и модели исполнения (например, нитевой код Forth), стековые
языки обзаводятся формальной базой, которая позволяет точно описывать
работу программ и проводить мощный статический анализ. В свою очередь
статический анализ открывает для стековых языков новые возможности.
Верификация кода и агрессивные оптимизации являются несомненно
большими преимуществами для низкоуровневых языков.

Одной из вершин достижений стековых языков можно считать Wasm.
Исходные цели проектирования предлагают идеальные для использования
стекового языка условия, на основе которых выросла система, сочетающая в
себе сразу несколько преимуществ, изначально считающихся несочетаемыми:
высокая портируемость под разные платформы (в сравнении с другими
виртуальными машинами), но при этом большая производительность
(например, в сравнении с традиционными движками ECMAScript) за счет
низкоуровневых интерфейсов и оптимизаций и верифицируемость (и оттого
частично безопасность). Wasm вбирает в себя теоретические и практические
наработки в области стековых языков, решая с помощью них сложную задачу
исполнения прикладного кода с учетом заданных требований.

В результате анализа можно сделать вывод, что современная ниша
стековых языков – это быстрые виртуальные машины, запускаемые на
широком диапазоне устройств. Это утверждение также подтверждается
фактом, что спецификации самых распространенных виртуальных машин
(JVM, .NET CLR, Wasm) используют стековую модель исполнения кода.
...
// Можно выделить закономерность. Если язык предназначен для некоего высокоуровневого взаимодействия, то он скорее имеет библиотеку, пусть и не самую большую (Joy~@joy-tut, Cat~@cat-b, Factor~@factor-paper). Если же речь идет про язык близкий к машине, то стандартной библиотеки у него нет, но есть иной способ подключать дополнительную функциональность (Forth~@forth-standard, Wasm~@wasm-spec).
...
// Области применения рассмотренных языков в значительной степени определяются тем, на какие ограничения и требования изначально ориентирована их модель исполнения. Forth применяется во встраиваемых и ресурсно-ограниченных системах и в системах реального времени, где наиболее характерны малый размер среды, предсказуемость поведения и возможность низкоуровневого управления аппаратурой через минимальное ядро и расширяемый словарь слов~@forth-page. Joy имеет преимущественно академическое и экспериментальное применение. Cat также относится к исследовательским языкам и изучает возможности верификации и оптимизации программ .NET CIL. Factor занимает нишу языка общего назначения: для него наиболее характерны развитая стандартная библиотека, инструменты разработки и оптимизирующая компиляция, при этом сохранён конкатенативный стиль и используется статический контроль формы стека (stack-checker) как средство повышения надёжности и производительности~@factor-paper~@factor-wiki. WebAssembly используется как переносимый низкоуровневый формат для безопасного исполнения недоверенного кода, для него наиболее характерна предсказуемая семантика и совместимость с различными исходными языками и рантаймами~@wasm-spec.

// Таким образом, по областям применения можно выделить характерные черты областей применимости стековых языков: для встраиваемых и системных задач наиболее важны минимализм и прямой контроль над ресурсами (Forth); для учебных и исследовательских задач -- формальная простота модели и возможность строгих рассуждений о композиции программ (Joy); для задач верификации и статического контроля -- типизация эффектов стека и вывод типов, позволяющие гарантировать корректность управления потоком и формы стека (Cat); для прикладного программирования общего назначения -- сочетание богатой библиотеки и инструментария с компиляцией и анализом, уменьшающими типичные риски стековой модели (Factor); для переносимого и изолированного исполнения -- стандартизованный байткод, однопроходная валидация и возможность эффективной компиляции в машинный код в рантайме (Wasm).

// // Можно выделить типичные области применимости языков: это языки для низкоуровневых виртуальных машин (Forth, Wasm), а также языки академической направленности (Joy, Cat). Factor можно выделить отдельно, как эксперимент по созданию прикладного стекового языка, и вынести в порядке исключения.

// // == Выводы

// В целом же в пути развития стековых языков видна явная тенденция: изначально работающие на высоких скоростях и низких уровнях абстракций благодаря простоте, высокой переносимости и оптимизациям, доступным фактически бесплатно за счет структуры программ и модели исполнения (например, нитевой код Forth), стековые языки обзаводятся мощной формальной базой, которая позволяет точно описывать работу программ и проводить мощный статический анализ. В свою очередь статический анализ открывает для стековых языков новые возможности. Верификация кода и агрессивные оптимизации являются несомненно большими преимуществами для низкоуровневых языков.
