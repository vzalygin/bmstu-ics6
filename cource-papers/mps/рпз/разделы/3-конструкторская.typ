...
Выбор микросхемы памяти AS6C1008, построенной на низковольтовых КМОП-транзистрах, обусловлен ее объемом, скоростями работы и требуемым питанием, а ее интерфейсом, совместимым с параллельным интерфейсом XMEM. Микросхема может хранить 128 кБайт информации, адресуемые при помощи 17 битов (в рамках устройство используется 16 бит, контакт старшего бита притянут к земле). Тайминги ответа микросхемы не превышают 50мс, а сама микросхема работает при диапазоне напряжений от 3 до 5,5 вольт. Набор перечисленных свойств делает ее подходящей под использование в курсовой работе.

Для поддержки интерфейса XMEM микросхеме памяти необходим регистр-защелка на 8 бит, коим выступает SN74HC573A. Регистр аналогично построен на технологии низковольтовых КМОП-транзисторов и питается напряжение в диапазоне от 3 до 5,5 вольт.
...
Сопряжение микроконтроллера с персональным компьютером по USB реализовано на микросхеме USB-UART преобразователя CH340C (DD5), которая с одной стороны имеет интерфейс UART, а с другой стороны -- интерфейс USB, таким образом позволяет установить связь с компьютером по протоколу UART. На её выводы UD+ и UD– подведены линии D+ и D– разъёма USB XP2, а вывод VCC подключён к шине +5 В. Встроенный стабилизатор ядра микросхемы питается от линии +5 В. Пин на питание +3 В развязан через керамический конденсатор на землю в соответствии с требованиями из документации.
