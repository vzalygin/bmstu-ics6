...
Последовательный интерфейс UART со стороны микроконтроллера реализуется линиями USART1. Вывод PD3 (TXD1) подключён к входу RXD микросхемы CH340C, а вывод PD2 (RXD1) — к её выходу TXD. Остальные выводы микросхемы CH340C, связанные с аппаратным управлением модемными сигналами (CTS, DSR, DTR и др.), в данной схеме не используются и остаются свободными или подтянутыми к фиксированным уровням согласно рекомендациям документации к микросхеме @datasheet-ch340.
