...
Согласно техническому заданию, необходимо разработать на основе микроконтроллера устройство озвучивания текста на русском и английском языках. Текст к микроконтроллеру должен передаваться по протоколу UART.

Микроконтроллер должен уметь работать с протоколом UART и считывать входящий текст, выполнять необходимые преобразования, а затем озвучивать его. Как и оригинальная программа SAM, устройство должно поддерживать флаги, передаваемые во входящем тексте (флаг обозначается при помощи тире на месте первого символа). Также устройство должно уметь подавать сигнал о готовности к вводу после инициализации, передавать информационные и отладочные сообщения по UART передающему устройству.

Поскольку встроенной в МК ОЗУ объёмом 4 кБ не хватает для генерации звукового потока, необходимо использовать внешнюю память, которая расширяет адресное пространство данных вплоть до 64 кБ, чего уже достаточно для работы алгоритмов синтеза речи. Согласно даташиту микроконтроллера, работа с внешней памятью должна осуществляться при помощи вспомогательного регистра-защёлки, который необходим для хранения младших бит адреса до окончания цикла чтения или записи @datasheet-atmega.

Для генерации звукового потока сигнал ШИМ, генерируемый МК, должен проходить несколько этапов обработки: сначала он поступает на RC-фильтр, который сглаживает высокочастотную составляющую, затем усиливается усилителем мощности, повышающим уровень сигнала по напряжению и току и обеспечивающим необходимую громкость динамика.

Для быстрого и простого соединения МК с компьютером устройство снабжается преобразователем интерфейсов UART-USB.

Питание устройство можно организовать при помощи того же самого USB-порта. Согласно спецификации интерфейса USB 2.0 напряжение на линии питания составляет 5 вольт @usb-spec, что подходит микроконтроллеру.
...
Отдельно в микроконтроллере присутствует энергонезависимая EEPROM-память объёмом 4 Кбайт. В курсовой работе она не используется.
...
Подключение внешней памяти реализует стандартный параллельный интерфейс XMEM, поддерживаемый ATMega128A. Линии порта A микроконтроллера используются как мультиплексированная шина AD0-AD7: они подключены одновременно ко входам D0-D7 регистра-защёлки SN74HC573A и к двунаправленной шине данных D0-D7 микросхемы SRAM AS6C1008. Сигнал ALE, формируемый выводом PG2, подаётся на вход LE регистра-защёлки как того требует документация микросхемы @datasheet-reg. В момент начала цикла обращения по шине на порт A выводится младший байт адреса, и при помощи стробирующего сигнала ALE фиксируется выходах Q0-Q7, которые подключены к адресным входам A0-A7 микросхемы SRAM @настройка-внешней-памяти.

Старшие адресные биты A8-A15 формируются выводами порта C микроконтроллера и напрямую подключены к соответствующим выводам памяти. Управляющие сигналы чтения и записи формируются выводами порта G: сигнал /RD (PG1) подаётся на вход /OE ОЗУ, а сигнал /WR (PG0) — на вход /WE. Вывод /CE микросхемы памяти подключён к общему проводу, а вывод CE2 — к шине +5 В, что обеспечивает постоянно разрешённое состояние микросхемы @datasheet-mem; фактический выбор режима работы определяется только сигналами /OE и /WE. Адресный вход A16 притянут к земле, поэтому используется младшая половина доступного адресного пространства ОЗУ. В цепях питания регистра-защёлки и памяти установлены развязывающие керамические конденсаторы C4 и C5.

Выбор микросхемы памяти случайного доступа AS6C1008, построенной на низковольтовых КМОП-транзистрах, обусловлен ее объемом, скоростями работы и требуемым питанием, а также ее интерфейсом, совместимым с параллельным интерфейсом XMEM. Микросхема может хранить 128 кБайт информации, адресуемые при помощи 17 битов (в рамках устройство используется 16 бит, контакт старшего бита притянут к земле). Тайминги ответа микросхемы не превышают 55нс @datasheet-mem (что позволяет обращаться к микросхеме на частоте 8 МГц), а сама микросхема работает при диапазоне напряжений от 2,7 до 5,5 вольт @datasheet-mem. Набор перечисленных свойств делает ее подходящей под использование в курсовой работе.

Для поддержки интерфейса XMEM микросхеме памяти необходим регистр-защелка на 8 бит, коим выступает SN74HC573A. Регистр аналогично построен на технологии низковольтовых КМОП-транзисторов и питается напряжением в диапазоне от 2 до 6 вольт @datasheet-reg. Для регистра скорость распространения сигнала составляет в среднем 21нс @datasheet-reg.

Таким образом микросхемы были подобраны на основе необходимого напряжения питания, разрядности, скорости ответа, чтобы подходить под режим работы ATMega128A на частоте 8 МГц без дополнительных согласований напряжений.
...
Примеры временных диаграмм циклов чтения и записи показаны на рисунке @xmem. В устройстве используются короткие циклы без дополнительных тактов задержек, что возможно благодаря быстродействию микросхем регистра-защелки и памяти.

#рис(image(
  "../материалы/xmem.png",
  width: 100%,
))[ Временная диаграмма XMEM циклов чтения и записи без дополнительных задержек ] <xmem>

=== Подключение модуля CH340C

Сопряжение микроконтроллера с персональным компьютером по протоколу UART реализовано на микросхеме USB-UART преобразователя CH340C (DD5), которая с одной стороны имеет интерфейс UART (TTL уровни напряжений), а с другой стороны -- интерфейс USB. На её выводы UD+ и UD- подведены линии D+ и D- разъёма USB XP2, а вывод VCC подключён к линии питания  +5 В. Встроенный стабилизатор ядра микросхемы питается от линии +5 В. Пин на питание +3 В развязан через керамический конденсатор на землю в соответствии с требованиями из документации @datasheet-ch340 -- в питании микросхемы пин не участвует.

Последовательный интерфейс UART со стороны микроконтроллера реализуется линиями USART1. Вывод PD3 (TXD1) подключён к входу RXD микросхемы CH340C, а вывод PD2 (RXD1) — к её выходу TXD. Остальные выводы микросхемы CH340C, связанные с аппаратным управлением модемными сигналами (CTS, DSR, DTR и др.), в схеме не используются и остаются свободными или подтянутыми к фиксированным уровням согласно рекомендациям документации к микросхеме @datasheet-ch340.
...
С выхода RC-фильтра (разрыв D на схеме) сигнал подаётся на вход INR усилителя мощности PAM8403 (DD4). Усилитель подобран согласно требуемому напряжению питания (5 вольт), а также выходной мощности (1,5 ватт на динамик) @datasheet-pam8403. Усилитель питается от линии +5 В, к его выводам питания подключены конденсаторы C2, C9 и C10, обеспечивающие развязку питания микросхемы согласно документации @datasheet-pam8403. В схеме используется только правый канал усилителя, пины второго кана притянуты к земле. Выход усилителя соединён к маленькому динамику FBS1850 (BA1) с сопротивлением 8 Ом @datasheet-d. Такой динамик подходит к усилителю, тк его входная мощность согласуется с мощностью усилителя.

Расчет RC-фильтра выполняется на основе следующих данных. Сопротивление резистора подбирается на основе напряжения питания и максимального тока на выходе GPIO-порта микроконтроллера ($I_"max" = 20 " мА"$ согласно даташиту на микроконтроллер @datasheet-atmega), тогда минимальное сопротивление резистора определяется законом Ома, описанной в формуле @ом. Для наличия запаса удобно использовать резистор $R_1 = 1 " кОм"$.  /*Частота ШИМ при работающем на частоте 8 МГц микроконтроллер будет $f_"PWM" = (8 "МГц")/(256) approx 31 "кГц"$.*/ Для речи достаточно взять частоту среза около 3-3.5 кГц (передача голоса по телефонной связи работает на полосе частот 300-3400 Гц @звук-мобильная-связь). Тогда можно вычислить емкость конденсатора при сопротивлении резистора $R_1 = 1 " кОм"$ и частоте среза $f_c = 3.2 " кГц"$. Вычисления представлены в формуле @конд.
...
$ C_3 = 1 / (2 pi dot R_1 dot f_c) = 1 / (2 pi dot 1000 " Ом" dot 3200 " Гц") approx 50 " нФ", $ <конд>
...
Для динамика мощность можно взять из даташита усилителя: $P_"динамик" = 1.5 "Вт"$ @datasheet-pam8403. 
...
$ P = I_"питания" dot U_"питания", $ <мощность-микросхем>
...
$
  P_"sum" = sum_i P_i = sum_i I_i dot U_"питания" + P_"RC" + P_"динамик" = \ = (10 + 0.08 + 10 + 3.5 + 20) " мА" dot 5 " B" + 25 " мВт" + 1.5 " Вт" approx \ approx 1.570 " Вт",
$ <сумм-мощн>
