...
Целевым микроконтроллером в данной работе является ATMega128A, относящийся к семейству высокопроизводительных 8-битных AVR. Он обладает расширенным объемом программной памяти (порядка 128 Кбайт Flash), внутренней ОЗУ и энергонезависимой EEPROM-памятью. К архитектурным особенностям данного микроконтроллера относятся наличие нескольких таймеров/счетчиков с поддержкой режима ШИМ, универсальных последовательных интерфейсов (USART, SPI, TWI), встроенного АЦП, а также внешнего интерфейса памяти XMEM. Наличие двух независимых USART, развитой системы прерываний и широкого диапазона тактовых частот обеспечивает гибкость при организации обмена с внешними устройствами и генерации звука.

В основе проекта лежит S.A.M. (The Software Automatic Mouth -- TODO ссылка) -- программа синтеза речи (TTS) для английского языка, созданная компанией Don't ask software в 1982 году для компьютеров Commodore C64, Atari 8-bit, а также Apple II. S.A.M. является одной из первых коммерческих программ синтеза речи, выпущенных на рынок. Программа включает в себя преобразователь текста в фонемы, который называется reciter, функцию преобразования фонем в речь для окончательного вывода (в формате .wav файла), а также набор функций для отладки. Изначально написанная на ассемблере под целевые компьютеры, в 2015 году благодаря стараниям Stefan Macke программа была дизассемблирована в код на языке C и выложена в открытый доступ без указания какой-либо лицензии (в связи с неуспешными попытками связаться с создателями программы, Don't ask software ныне не существует). Программа обладает поразительно малыми размерами -- "боевая" автономная версия программы под x86 занимает менее 39 кБ дискового пространства и требует менее 128 кБ оперативной памяти для работы (в основном занимаемой выходным .wav файлом). Данная особенность программы позволяет портировать ее даже для самых маленьких встроенных систем. В рамках данной курсовой работы ставится задача портирования S.A.M. под МК ATMega128A.

С точки зрения проекта микроконтроллер ATMega128A обладает всеми необходимыми свойствами:

- наличие 128 кБ памяти программы;
- ОЗУ размером в 4 кБ, которое расширяется до 64 кБ при использовании внешней памяти и протоколу XMEM;
...
- простота и открытость архитектуры МК семейства AVR для тонкой настройки.
