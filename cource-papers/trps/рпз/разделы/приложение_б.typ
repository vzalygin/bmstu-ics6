...
#show: приложение.with(буква: "Б", содержание: [ ПРИЛОЖЕНИЕ Б. Руководство программиста ])
...
#страница(image("../материалы/титул_руководство_программиста.svg"), номер: нет, содержимое: [#context counter(page).update(1)])

#h(1.25cm) #нумерованный_заголовок(номер: 1, содержание: нет)[ 
...
Листинг @факт содержит в себе пример программы на исходном языке, вычисляющей факториал:
...
.                    # print the result
```)[ Пример программы, вычисляющей факториал ] <факт>


#pagebreak()
#context counter(page).update(5 + counter(page).at(label("titul-b")).at(0))
