...
В качестве основной стратегиии выбрано функциональное тестирование. Данная стратегия основывается на принципе "черного ящика", что делает ее максимально близкой к опыту, который получает пользователь при использовании программного решения.
...
Таблица @сквозные-тесты показывает перечень тестов по методу эквивалентного разбиения. 
...
#таблица(table(
  ...
  [ 26 ], [ Корректная трансляция бинарной операции ], [ Нет ], [ 1b -1b 0b . . . ], [ Исполняемый файл, вывод: 1 1 0 ], [ Исполняемый файл, вывод: 1 1 0 ], [ Корректная \ работа ],
  ...
  [ 31 ], [ Корректная трансляция операции создания именнованных значений ], [ Нет ], [ 1 :foo ], [ Исполняемый файл, вывод: пустой вывод ], [ Исполняемый файл, вывод: пустой вывод ], [ Корректная \ работа ],
  [ 32 ], [ Корректная трансляция операции применения именнованных \ значений ], [ Нет ], [ 1 :foo foo . ], [ Исполняемый файл, вывод: 1 ], [ Исполняемый файл, вывод: 1 ], [ Корректная \ работа ],
...
При использовании метода покрытия операторов важно написать тесты,покрывающие все операторы тестируемого компонента. В таблице @модульные-тесты приведены результаты структурного тестирования.
