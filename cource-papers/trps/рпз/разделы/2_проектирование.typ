...
Согласно подразделу @анал-задания в рамках разработки был выбран структурный подход. Для работы при таком подходе необходимо уточнить структурную схему программного решения. На рисунке @структурная-схема изображена структурная схема проекта, составленная по включению.

#рис(image("../материалы/структурная схема.png", width: 80%))[ Структурная схема ] <структурная-схема>

Описание частей структурной схемы приведено ниже:

- программа-компилятор, агрегирующая все модули программы, является точкой входая при запуске программы;
...
- компонент разбора текстов, включает в себя подпрограммы для разрбора текстов на исходном языке;
- компонент компиляции, содержит подпрограммы, участвующие в процессе компиляции абстрактного синтаксического дерева в язык ассемблера целевого набора команд;
...
Согласно техническому заданию приложение должно иметь интерфейс командной строки. Для наглядности используется синтаксическая диаграмма грамматики интерфейса. Грамматика показа с использованием расширенной формы Бекуса-Наура (РБНФ). Форма изображена на рисунке @рбнф. Аксиомой грамматики является нетерминал "plc".

#рис(image(
  "../материалы/курсач_трпс-ОК РБНФ.drawio.png", width: 100%
))[ Описание интерфейса командной строки в виду РБНФ ] <рбнф>

Интерфейс командной строки соответствует принятым идиомам проектирования интерфейсов для консольных приложений @cli-guidelines. Интерфейс состоит из ключевого слова "plc", служащего именем приложения и началом команд для него, из набора флагов, определяющих поведение приложения, из имен файлов, которыми должна оперировать программа.
...
В подразделе @анал-процессов описаны функциональные диаграммы процессов, в рамках которых используется решение. Для переноса процессов в программное обеспечение необходимо описать алгоритмы, соответствующие процессам. В качестве представления алгоритмов целесообразно использовать схемы алгоритмов. На рисунке @схема-алго-1 представлен алгоритм для основной подпрограммы. В рамках основной подпрограммы происходит выбор режима, а также вызов агрегирующей подпрограммы "выполнить".

Схема алгоритма работы библиотечной подпрограммы "выполнить" представлена на рисунке @схема-алго-2. В зависимости от режима работы подпрограмма выполняет разное количество шагов для создания результирующего выходного файла. Для выполнения промежуточных шагов используются временные файлы. После проведения необходимых операций и получения выходного файла временные файлы удаляются.

#рис(image("../материалы/курсач_трпс-1 Схемы алгоритмов.drawio.png", height: 87%))[ Алгоритм работы основной подпрограммы ] <схема-алго-1>

#рис(image("../материалы/курсач_трпс-2 Схемы алгоритмов.drawio.png", height: 97%))[ Схема алгоритма подпрограммы "выполнить"] <схема-алго-2>

Поскольку язык обладает стандартной библиотекой (подробнее этом изложено в подразделе @компилятор), существует подпрограмма подготовки к компоновке файла стандартной библиотеки. Алгоритм данной подпрограммы показан на рисунке @схема-алго-3. Алгоритм соотвествует аналогичному для кода из входного файла, но оперирует заранее заготовленными ассемлерными листингами для создания объектного файла.
...
== Разработка компонента разрбота текста <парсер>

Выбранная библиотека для построения генераторов комбинаторных парсеров, Nom, позволяет реализовать разбор выражений методом рекурсивного спуска @документация-nom. Также по техническому заданию необходимо реализовать разбор ряда конструкций в синтаксисе обратной польской записи. В связи с данными ограничениями аксиома языка описывает подходящий под требования конкатенативный язык. Аксиома изображена на рисунке @аксиома. Определение аксиомы утверждает, что каждый "терм" окружен либо другими "термами", либо разделителями, либо началом и окончанием файла.

#рис(image(
  "../материалы/курсач_трпс-1 Синтаксическая диаграмма.drawio.png"
))[ Синтаксическая диаграмма аксиомы исходного языка ] <аксиома>

Одним из главных правил в грамматике является правило "терм", обозначающее некоторую операцию. Рисунок @терм показывает синтаксическую диаграмму правила "терм". "Терм" означает синтаксическую единицу, команду на исходном языке. 

#рис(image(
  "../материалы/курсач_трпс-2 Синтаксическая диаграмма.drawio.png", width: 110%
...
Остальные правила изображены на рисунках @число, @термы, @список. Данные правила задают непосредственно команды исходного языке. На рисунке @список представлена синтаксическая диаграмма списков -- важного элемента языка, который, являясь рекурсивным вкупе с операцией ветвления, позволяет языку удовлетворить полноте по Тьюрингу.

#рис(image(
  "../материалы/курсач_трпс-3 Синтаксическая диаграмма.drawio.png", width: 90%
))[ Синтаксические диаграммы: а -- "разделитель"; б -- "комментарий" ] <разделитель>

#рис(image(
  "../материалы/курсач_трпс-4 Синтаксическая диаграмма.drawio.png", width: 80%
))[ Синтаксическая диаграмма "число" ] <число>

#рис(image(
  "../материалы/курсач_трпс-5 Синтаксическая диаграмма.drawio.png", width: 110%
))[ Синтаксические диаграммы правил для некоторых термов ] <термы>

#рис(image(
  "../материалы/курсач_трпс-6 Синтаксическая диаграмма.drawio.png", width: 110%
))[ Синтаксические диаграммы правил: а -- "список"; б -- "привязать к имени"; в -- "положить именнованное значение"; г -- "имя" ] <список>
...
== Разработка компонента компиляции <компилятор>

На основе АСТ, полученного после разбора исходного кода (процесс описан в подразделе @парсер), создается ассемблерный листинг программы. Чтобы выполнить данную задачу, необходимую каждой операции сопоставить заготовку (шаблон) на языке ассемблера, удовлетворяющую семантике операции @слайды. Именно при разработке таких заготовок решается задача адаптации стекового языка (предназначенного для стековой машины) под регистровую машину.
...
Для последующего развития предусмотрена стандартная библиотека. В текущей версии решения реализованы функции ввода/вывода, завершения работы приложением. Библиотека собирается аналогично исходному коду в объектный файл, а затем компонуется с объектным файлом с точкой входа программы.
...
i!(Mov, indirect_register!(Ebx), reg!(Eax)),
    ``` ],
    [ Вывести ], [  #set table(stroke: 0em) 
    ```
i!(Call, oplabel!(STD_PRINT_FN_LABEL))
    ``` ],
    [ Дублировать ], [  #set table(stroke: 0em) 
    ```
i!(Mov, reg!(Eax), indirect_register!(Ebx)),
i!(Sub, reg!(Ebx), Op::Literal(OP_SIZE_BYTES)),
i!(Mov, indirect_register!(Ebx), reg!(Eax)),
    ``` ],
    [ Удалить ], [  #set table(stroke: 0em) 
    ```
i!(Add, reg!(Ebx), Op::Literal(OP_SIZE_BYTES))
    ``` ],
    [ Вытащить ], [  #set table(stroke: 0em) 
    ```
...
i!(label!(no_exch_label.as_str())),
    ``` ],
    [ Применить список ], [  #set table(stroke: 0em) 
    ```
i!(Add, reg!(Ebx), Op::Literal(OP_SIZE_BYTES)),
i!(Call, opexpr!(format!("[EBX-{OP_SIZE_BYTES}]"))),
    ``` ],
    [ Преобразовать в булево значение ], [  #set table(stroke: 0em) 
    ```
...
i!(Mov, indirect_register!(Ebx), reg!(Eax)),
    ``` ],
    [ Отрицание ], [  #set table(stroke: 0em) 
    ```
...
i!(Add, indirect_register!(Ebx), reg!(Eax)),
    ``` ],
    [ Привязать значение к имени ], [  #set table(stroke: 0em) 
    ```
...
i!(Mov, opexpr!(format!("[{name}]")), reg!(Rax)),
    ``` ],
    [ Положить именнованное значение ], [  #set table(stroke: 0em) 
    ```
i!(Mov, reg!(Rax), opexpr!(format!("[{name}]"))),
i!(Sub, reg!(Ebx), Op::Literal(OP_SIZE_BYTES)),
i!(Mov, indirect_register!(Ebx), reg!(Rax)),
    ``` ],
  ))[ Некоторые операции и их ассемблерные шаблоны ] <ассемблер-шаблоны> 
]
