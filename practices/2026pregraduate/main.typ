...
#страница(image("задание.jpg", height: 100%), номер: нет)
...
Байткод -- бинарное представление инструкций для машины
...
Интепретатор -- программа, которая выполняет исходный код, покомандно анализируя и исполняя команды "на лету"
...
Конкатенативный язык -- язык, в котором для композиция функций выражается через их конкатентацию
...
Рантайм -- (анг. Runtime, время исполнения) имеет несколько знчений; обозначение времени исполнения программы; окружение программы, в котором она исполняется
...
S-выражение -- скобочный формат записи, где операция представлена скобками, внутри которых на первом месте стоит оператор, а затем перечисляются операнд
...
Целью преддипломной практики является проведения ряда аналитических работ применительно к теме ВКРБ "компилятор стекового языка программирования". В процессе работ выполняются аналитический обзор стековых языков программирования, сравнительный анализ стековых языков программирования, анализ средств вывода типов стековых языков программирования, анализ устройства байткода и модели исполнения WebAssembly.

Стековые (или стек-ориентированные) языки программирования характеризуются применением стека данных в качестве основного механизма передачи информации и хранения результатов вычислений.

Модель исполнения стекового языка -- это некая (чаще всего виртуальная) стековая машина, имеющая стек данных, операционный стек и набор команд для работы со стеком. Программа представляет собой последовательность исполняемых команд, которые могут быть либо встроенными примитивами (например, арифметические операции, операции над стеком; интринсиками), либо определенными пользователем субпрограммами. В терминах стековых языков команды принято называть словами, или комбинаторами, или функциями. Каждое слово либо модифицирует состояние стека, либо изменяет поток исполнения программы. Большинство команд в такой машине берёт необходимые операнды с вершины стека, выполняет вычисление и кладёт полученный результат обратно на стек.

= Аналитический обзор стековых языков программирования

Начиная с семидесятых годов прошлого века было создано некоторое количество стековых языков. Далее рассмотрены несколько из них, каждый из которых привнес нововведения в области стековых языков. В обзор не включены, однако также заслуживают внимания:
...
Статическая типизация в контексте стековых языков подразумевает, что в момент компиляции возможно построить конфигурацию стека (его размер и "форма"~-- типы значений, которые на нем лежат) для произвольного момента времени исполнения. Таким образом вывод типов открывает путь к статическому анализу и проведению трансформаций. Подробности статического вывода типов описываются в разделе~@заголовок-вывод-типов. В языке присутствует несколько встроенных команд, их описание приведено в таблице~@команды-cat. Помимо них в язык встроены арифметические, логические, команды и команды сравнения чисел. Язык использует три типа данных~-- числа, булевы значения и цитаты.
